...
import { getRelationships, getAudioFileById, getMarkdownFileById } from '../utils/database';
    ...
    <div className="markdown-viewer-overlay" onClick={onClose}>
      <div className="markdown-viewer" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="markdown-header">
          <h1>{markdown.title || markdown.fileName}</h1>
          <p className="markdown-meta">
            <i className="fas fa-calendar"></i> {new Date(markdown.dateAdded).toLocaleString('pt-BR')}
          </p>
        </div>
        ...
        {relatedFiles.length > 0 && (
          <div className="related-files-section">
            <h3>
              <i className="fas fa-link"></i> Arquivos Relacionados ({relatedFiles.length})
            </h3>
              ...
              {relatedFiles.map((file) => (
                ...
                >
                  <div className="related-file-icon">
                    <i className={`fas fa-${file.type === 'audio' ? 'music' : 'file-alt'}`}></i>
                  </div>
                  <div className="related-file-info">
                    <h4>{file.type === 'audio' ? file.fileName : (file.title || file.fileName)}</h4>
                    <p>{file.type === 'audio' ? 'Arquivo de Áudio' : 'Documento Markdown'}</p>
                  </div>
        ...
        {loading && (
          <div className="loading-related">
            <i className="fas fa-spinner fa-spin"></i> Carregando relacionamentos...
          </div>
        )}
      </div>
    </div>
