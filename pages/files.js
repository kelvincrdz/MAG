...
export default function FilesPage() {
  ...
  async function carregarArquivos() {
    ...
    try {
      ...
      setAudios(audioData.sort((a, b) => new Date(b.dateAdded) - new Date(a.dateAdded)));
      setMarkdowns(mdData.sort((a, b) => new Date(b.dateAdded) - new Date(a.dateAdded)));
      setMags(magData.sort((a, b) => new Date(b.dateProcessed) - new Date(a.dateProcessed)));
  ...
  async function handleDelete(id, type) {
    if (!confirm(`Deseja realmente excluir este ${type === 'audio' ? 'áudio' : 'markdown'}?`)) {
      return;
    }
  ...
  const audiosFiltrados = filtro === 'todos' || filtro === 'audios' ? audios : [];
  const markdownsFiltrados = filtro === 'todos' || filtro === 'markdowns' ? markdowns : [];
          ...
          <>
            {audiosFiltrados.length > 0 && (
              <div className="files-section">
                <h2><i className="fas fa-music"></i> Arquivos de Áudio ({audiosFiltrados.length})</h2>
            ...
            {markdownsFiltrados.length > 0 && (
              <div className="files-section">
                <h2><i className="fas fa-file-alt"></i> Arquivos Markdown ({markdownsFiltrados.length})</h2>
                <div className="files-grid">
                  {markdownsFiltrados.map((md) => (
                    <div key={md.id} className="file-card markdown-card">
                      ...
                      <div className="file-info">
                        <h3>{md.title || md.fileName}</h3>
                        <p className="file-preview">{md.content.substring(0, 100)}...</p>
                        <p className="file-meta">
                          <span>{formatDate(md.dateAdded)}</span>
                        </p>
                        {relationships[md.id] && relationships[md.id].length > 0 && (
                          <p className="file-links">
                            <i className="fas fa-link"></i> {relationships[md.id].length} vínculo(s)
                          </p>
                        )}
                      </div>
                      <div className="file-actions">
            ...
            {audiosFiltrados.length === 0 && markdownsFiltrados.length === 0 && (
              <div className="empty-state">
                <i className="fas fa-folder-open"></i>
                <h3>Nenhum arquivo encontrado</h3>
                <p>Faça upload de um arquivo .mag no player para começar</p>
                <button className="btn-primary" onClick={() => router.push('/player')}>
                  Ir para o Player
                </button>
              </div>
            )}
